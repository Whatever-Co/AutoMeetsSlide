...
# Find Sparkle tools - check custom DerivedData first, then Xcode default
SPARKLE_BIN="${BUILD_DIR}/DerivedData/SourcePackages/artifacts/sparkle/Sparkle/bin"
if [[ ! -x "${SPARKLE_BIN}/generate_appcast" ]]; then
  # Fallback to Xcode default DerivedData
  SPARKLE_BIN=$(find ~/Library/Developer/Xcode/DerivedData/AutoMeetsSlide-*/SourcePackages/artifacts/sparkle/Sparkle/bin -name "generate_appcast" -exec dirname {} \; 2>/dev/null | head -1)
fi
...
DMG_NAME="AutoMeetsSlide-${VERSION}.dmg"
...
for TAG in $RELEASES; do
  ...
  PREV_DMG_NAME="AutoMeetsSlide-${TAG#v}.dmg"
...
"${SPARKLE_BIN}/generate_appcast" \
  --download-url-prefix "https://github.com/Whatever-Co/AutoMeetsSlide/releases/download/v${VERSION}/" \
  "$APPCAST_DIR"
